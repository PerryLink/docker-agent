...
func TestGetLastMessageContent_Empty(t *testing.T) {
	t.Parallel()
	s := &Session{}
	assert.Equal(t, "", s.GetLastAssistantMessageContent())
	assert.Equal(t, "", s.GetLastUserMessageContent())
}
...
func TestGetLastMessageContent_SystemFiltered(t *testing.T) {
	...
	assert.Equal(t, "", s.getLastMessageContentByRole(chat.MessageRoleSystem))
}
...
func TestGetLastMessageContent_BlankStopsSearchInSubSession(t *testing.T) {
	...
	assert.Equal(t, "", parent.GetLastAssistantMessageContent())
}
...
func newLmcRng(seed int64) *lmcRng { return &lmcRng{state: seed*6364136223846793005 + 1442695040888963407} }
...
var lmcRoles = []chat.MessageRole{chat.MessageRoleUser, chat.MessageRoleAssistant, chat.MessageRoleSystem, chat.MessageRoleTool}
var lmcContents = []string{"", " ", "\n\t ", "hello", "  padded  ", "multi\nline", "x"}
